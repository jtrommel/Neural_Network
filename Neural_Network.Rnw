...
x3D <- seq(min(hotdog$x1 - 1), max(hotdog$x1 + 1),by = 0.25)
y3D <- seq(min(hotdog$x2 - 1), max(hotdog$x2 + 1),by = 0.25)
z3D <- matrix(logreg$coefficients[1] + logreg$coefficients[2]*rooster$x1 + logreg$coefficients[3]*rooster$x2, nrow=length(x3D), ncol=length(y3D))
